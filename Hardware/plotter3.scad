...
difference() {
	...
	translate([11,-15,-5]) cylinder(r=8, h=10);
	translate([20,36,-5]) cylinder(r=25, h=10);
	translate([-28,14,-5]) cylinder(r=8, h=10);
	//translate([36,18,-5]) cylinder(r=10, h=10);
	translate([-28,40,-5]) cylinder(r=15, h=10);
	//translate([36,47,-5]) cylinder(r=9, h=10);
...
difference() {
	union() {
		translate([-6.0,-plate_h/2,0]) cylinder(r=5,h=16);
		translate([6.0,-plate_h/2,0]) cylinder(r=5,h=16);
		...
		translate([-0.5,-plate_h/2+1.5,0]) cube([1,29,16]);
	}
...
//		translate([0,-plate_h/2-23/2-4,plate_t/2+4]) cube(center=true, [40,6,4.5]);		
		translate([0,-plate_h/2-23/2-4,plate_t/2+4]) rotate([0,90,0]) rotate([0,0,30]) cylinder($fn=6,center=true, r=3, h=40);		
	}
...
//sd
module sd_holes(make_distances=0) {
	holes = [
		[  2, 2,-5 ],
		[  2, sd_h-2, -5 ],
		[  sd_w-2, 2, -5 ],
