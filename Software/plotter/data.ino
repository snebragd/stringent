...
bool getDataInternal(int plotNo, int point, float *x, float* y, int* pen)
{
  ...
  if (currentlySelectedPlot != plotNo) {
    //first call, check if we have plot stored on SD
      ...
      svgName[1] = '0' + plotNo;
      Serial.println("SVG Name=");
      Serial.println(svgName);



    if (spiffsActive) {
      if (SPIFFS.exists(svgName)) {
        svgFile = SPIFFS.open(svgName, "r");
        if (!svgFile) {
          Serial.print("Unable To Open '");
          Serial.print(svgName);
          Serial.println("' for Reading");
          Serial.println();
        } else {
          Serial.print("Opened '");
          Serial.print(svgName);
          Serial.print("' for Reading. File size is ");
          Serial.print(svgFile.size());
          Serial.println();
        }

        String s;
        while (svgFile.position() < svgFile.size())
        {
          s = svgFile.readStringUntil('\n');
          s.trim();
        }
      } else {
  ...
  if (svgFile) {
    ...
    bool res  = getSvgData(plotNo, point, x, y, pen);
...
seekTo(char* pattern)
{
  ...
  char* tmp = pattern;
  while (*tmp != '\0' && svgFile.position() < svgFile.size()) {
    char c = svgFile.read();
...
seekToPathStart(bool rewindFile) {
  ...
  if (!seekTo((char*)"<path")) {
    ...
    return false;
  } else if (svgFile.position() >= svgFile.size()) {
    return false;
  }
